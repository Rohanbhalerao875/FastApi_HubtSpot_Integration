...
async def authorize_hubspot_integration(user_id: str = Form(...), org_id: str = Form(...), item_type: str = Form(default="contact")):
    return await authorize_hubspot(user_id, org_id, item_type)
...
async def get_hubspot_credentials_integration(user_id: str = Form(...), org_id: str = Form(...), item_type: str = Form(default="contact")):
    return await get_hubspot_credentials(user_id, org_id, item_type)

@app.post('/integrations/hubspot/load')
async def get_hubspot_items_integration(credentials: str = Form(...), item_type: str = Form(default="contact")):
    return await get_items_hubspot(credentials, item_type)
