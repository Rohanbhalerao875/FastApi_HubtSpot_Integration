// slack.js

import { useState, useEffect } from 'react';
import {
    Box,
    Button,
    CircularProgress,
    Typography,
    FormControl,
    InputLabel,
    Select,
    MenuItem
} from '@mui/material';
import axios from 'axios';

export const SlackIntegration = ({ user, org, integrationParams, setIntegrationParams }) => {
    const [isConnected, setIsConnected] = useState(false);
    const [isConnecting, setIsConnecting] = useState(false);
    const [dataType, setDataType] = useState('channels');

    // Function to open OAuth in a new window
    const handleConnectClick = async () => {
        try {
            setIsConnecting(true);
            const formData = new FormData();
            formData.append('user_id', user);
            formData.append('org_id', org);
            const response = await axios.post(`http://localhost:8000/integrations/slack/authorize`, formData);
            console.log(response);
            const authURL = response?.data?.authorization_url;

            const newWindow = window.open(authURL, 'Slack Authorization', 'width=600, height=600');

            // Polling for the window to close
            const pollTimer = window.setInterval(() => {
                if (newWindow?.closed !== false) { 
                    window.clearInterval(pollTimer);
                    handleWindowClosed();
                }
            }, 200);
        } catch (e) {
            setIsConnecting(false);
            console.error("Slack connection error:", e);
            alert(e?.response?.data?.detail || "Error connecting to Slack");
        }
    }

    // Function to handle logic when the OAuth window closes
    const handleWindowClosed = async () => {
        try {
            const formData = new FormData();
            formData.append('user_id', user);
            formData.append('org_id', org);
            const response = await axios.post(`http://localhost:8000/integrations/slack/credentials`, formData);
            const credentials = response.data; 
            if (credentials) {
                setIsConnecting(false);
                setIsConnected(true);
                setIntegrationParams(prev => ({ 
                    ...prev, 
                    credentials: { user_id: user, org_id: org },
                    type: 'Slack',
                    dataType: dataType
                }));
            }
            setIsConnecting(false);
        } catch (e) {
            setIsConnecting(false);
            console.error("Slack credentials error:", e);
            alert(e?.response?.data?.detail || "Error getting Slack credentials");
        }
    }

    // Handle data type change
    const handleDataTypeChange = (event) => {
        const newDataType = event.target.value;
        setDataType(newDataType);
        if (isConnected) {
            setIntegrationParams(prev => ({
                ...prev,
                dataType: newDataType
            }));
        }
    };

    useEffect(() => {
        setIsConnected(integrationParams?.credentials ? true : false);
        if (integrationParams?.dataType) {
            setDataType(integrationParams.dataType);
        }
    }, [integrationParams]);

    return (
        <>
        <Box sx={{mt: 2, width: '100%'}}>
            <Typography variant="h6" gutterBottom>
                Slack Integration
            </Typography>
            
            <Box sx={{mt: 2, mb: 2}}>
                <FormControl fullWidth>
                    <InputLabel id="slack-data-type-label">Data Type</InputLabel>
                    <Select
                        labelId="slack-data-type-label"
                        id="slack-data-type"
                        value={dataType}
                        label="Data Type"
                        onChange={handleDataTypeChange}
                    >
                        <MenuItem value="channels">Channels</MenuItem>
                        <MenuItem value="users">Users</MenuItem>
                        <MenuItem value="messages">Messages</MenuItem>
                    </Select>
                </FormControl>
            </Box>
            
            <Box display='flex' alignItems='center' justifyContent='center' sx={{mt: 2}}>
                <Button 
                    variant='contained' 
                    onClick={isConnected ? () => {} : handleConnectClick}
                    color={isConnected ? 'success' : 'primary'}
                    disabled={isConnecting}
                    style={{
                        pointerEvents: isConnected ? 'none' : 'auto',
                        cursor: isConnected ? 'default' : 'pointer',
                        opacity: isConnected ? 1 : undefined
                    }}
                >
                    {isConnected ? 'Slack Connected' : isConnecting ? <CircularProgress size={20} /> : 'Connect to Slack'}
                </Button>
            </Box>
        </Box>
      </>
    );
}
